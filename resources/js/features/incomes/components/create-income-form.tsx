...
import { INCOME_FIELDS, INCOME_TYPE_OPTIONS, PERMANENT_TYPE_OPTIONS } from '../income-fields';
...
export default function CreateIncomeForm({ onCancel, onCreated }: { onCancel: () => void; onCreated?: () => void }) {
    ...
    const showMonthDay = isPermanent && (form.data.permanent_type === 'monthly' || form.data.permanent_type === 'yearly');
    ...
    return (
        <form onSubmit={form.submit} className="space-y-1">
            <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                    <h2 className="text-xl font-bold text-slate-950 dark:text-white">افزودن درآمد</h2>
                    <p className="mt-1 text-xs leading-6 text-slate-400">
                        اطلاعات درآمد را وارد کنید. فیلدهای زمان‌بندی بر اساس نوع درآمد به‌صورت خودکار نمایش داده می‌شوند.
                    </p>
                </div>
            ...
            <div className="grid gap-5 sm:grid-cols-2">
                <FieldShell label={INCOME_FIELDS.title.label} icon={INCOME_FIELDS.title.icon} error={form.errors.title}>
                    <input
                        name="title"
                        type="text"
                ...
                <FieldShell label={INCOME_FIELDS.amount.label} icon={INCOME_FIELDS.amount.icon} error={form.errors.amount}>
                    <div className="relative">
                        ...
                        />
                        <span className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 text-xs font-bold text-slate-400">تومان</span>
                    </div>
                </FieldShell>
            </div>
            ...
            <div className="pt-5">
                ...
                >
                    <ChoiceCards value={form.data.type} options={INCOME_TYPE_OPTIONS} onChange={form.setType} />
                </FieldShell>
            </div>

            <ConditionalField show={isPermanent}>
                <FieldShell label={INCOME_FIELDS.permanent_type.label} icon={INCOME_FIELDS.permanent_type.icon} error={form.errors.permanent_type}>
                    <ChoiceCards value={form.data.permanent_type} options={PERMANENT_TYPE_OPTIONS} onChange={form.setPermanentType} columns={4} />
                </FieldShell>
            </ConditionalField>
            ...
            <ConditionalField show={isPermanent && form.data.permanent_type === 'monthly'}>
                <div className="grid justify-items-center sm:grid-cols-2">
                    <FieldShell label={INCOME_FIELDS.permanent_day.label} icon={INCOME_FIELDS.permanent_day.icon} error={form.errors.permanent_day}>
                        <DayOfMonthField value={form.data.permanent_day} onChange={(value) => form.setData('permanent_day', value)} />
                    </FieldShell>

                    <FieldShell label={INCOME_FIELDS.permanent_time.label} icon={Clock3} error={form.errors.permanent_time}>
                        <TimePicker value={form.data.permanent_time} onChange={(value) => form.setData('permanent_time', value)} />
                    </FieldShell>
                </div>
            </ConditionalField>
            ...
            <ConditionalField show={showYearMonth}>
                <div className="grid justify-items-center gap-5 sm:grid-cols-3">
                    ...
                    <FieldShell label={INCOME_FIELDS.permanent_month.label} icon={INCOME_FIELDS.permanent_month.icon} error={form.errors.permanent_month}>
                        <MonthField value={form.data.permanent_month} onChange={(value) => form.setData('permanent_month', value)} />
                    </FieldShell>
            ...
            <ConditionalField show={isPermanent && (form.data.permanent_type === 'daily' || form.data.permanent_type === 'weekly')}>
                <FieldShell label={INCOME_FIELDS.permanent_time.label} icon={Clock3} error={form.errors.permanent_time}>
                    <TimePicker value={form.data.permanent_time} onChange={(value) => form.setData('permanent_time', value)} />
                </FieldShell>
            </ConditionalField>
            ...
            <ConditionalField show={isTemporary}>
                <FieldShell label={INCOME_FIELDS.temporary_at.label} icon={INCOME_FIELDS.temporary_at.icon} error={form.errors.temporary_at}>
                    <PersianDateTimePicker value={form.data.temporary_at} onChange={(value) => form.setData('temporary_at', value)} />
                </FieldShell>
            </ConditionalField>

            <div className="pt-5">
                <FieldShell label={INCOME_FIELDS.description.label} icon={INCOME_FIELDS.description.icon} error={form.errors.description}>
                    <textarea
                        name="description"
                        ...
                        rows={4}
                        className="max-h-50 min-h-20 w-full resize-y rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm leading-7 text-slate-900 transition outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-white/10 dark:bg-white/[0.04] dark:text-white"
                    />
                </FieldShell>
            </div>
            ...
            {!form.canSubmit && form.data.type !== '' && (
                <p className="mt-3 text-center text-xs text-slate-400">برای فعال شدن دکمه ذخیره، فیلدهای ضروری را کامل کنید.</p>
            )}
        </form>
    );
}
